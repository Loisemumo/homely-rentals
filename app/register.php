<?php
	...
	if(isset($_POST['register_apartment'])) {
			...
			try {
				$stmt = $connect->prepare('INSERT INTO house_registrations (fullname, email, mobile, place, price, apartment_name, number_of_bedrooms,  number_of_bathrooms, apartment_type,  image,  vacant, user_id) VALUES (:fullname, :email, :mobile, :place, :price, :apartment_name, :number_of_bedrooms,  :number_of_bathrooms, :apartment_type,  :image,  :vacant, :user_id)');
				...
				foreach ($_POST['ap_number_of_plats'] as $key => $value) {
					...
					$stmt->execute(array(
						...
						':number_of_bedrooms' => $_POST['number_of_bedrooms'][$key],
						':number_of_bathrooms' => $_POST['number_of_bathrooms'][$key],
						':place' => $_POST['place'][$key],						
						':price' => $_POST['price'][$key],
						':apartment_type' => $_POST['apartment_type'][$key],
						':image' => $target_file,
